...
class BatchRestoreValueUseCase:
    ...
    def execute(
        ...
        try:
            ...
            log(f"开始批量还原 value: {folder_path}")
            progress(0.05)
            ...
            for root, _, files in os.walk(folder_path):
                for filename in files:
                    ...
                    try:
                        ...
                        data = self.file_handler.restore_value_to_json(data)
                        ...
                        success += 1
                        log(f"{filename} → 已还原value（转为对象格式）")
            ...
            progress(1.0)
            log(f"还原完成: 总计{total}个 | 成功{success}个")
