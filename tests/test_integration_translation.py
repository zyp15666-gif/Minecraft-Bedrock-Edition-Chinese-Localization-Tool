...
def create_mock_strategy(term_service=None, quality_checker=None, cache=None, api_client=None):
    """创建测试用翻译策略"""
...
class TestStage1DirectTranslation:
    ...
    def test_stage1_success_caches_result(self, mock_post):
        ...
        strategy = create_mock_strategy(term_service, quality_checker, cache, api_client=api_client)
...
@pytest.mark.skip(reason="TranslationStrategy 已简化为按颜色码分阶段；提示词式二阶段回退由 APIManager/质量检查覆盖")
class TestStage2Fallback:
    """阶段2回退测试（历史场景，策略层已不再实现该分支）"""
...
class TestAPIRetry:
    """APIClient 单次请求语义（重试由 APIManager + unified_retry 统一处理）"""

    def test_api_client_propagates_connection_error(self):
        import requests
        ...
        with patch("api.api_client.requests.post") as mock_post:
            ...
            with pytest.raises(requests.exceptions.ConnectionError):
                api_client.translate(api_config, "Hello World", is_test=False)
            assert mock_post.call_count == 1

    def test_api_client_propagates_timeout(self):
        import requests
...
class TestLoadBalancing:
    ...
    def test_load_balancer_records_response_times(self):
        """测试负载均衡器记录响应时间"""
        lb = LoadBalancer()

        lb.record_response_time("api3", 0.12)
        lb.record_response_time("api3", 0.18)

        stats = lb.get_stats()

        assert "api3" in stats
        assert stats["api3"]["avg_response_time"] > 0
